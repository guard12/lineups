...
export const ImportPlayers = () => {
  ...
  const handleImportPlayers = (activityId: number) => {
    ...
    if (!activity) return;
    const players = activity.activities_users.filter((activityUser: HoldsportPlayer) => activityUser.status === 'Attending');
    importPlayers(players);
  };

  return (
    <div>
      <h1>Import Players from Holdsport</h1>
      <Input name="email" placeholder="Holdsport email" value={email} onChange={(e) => setEmail(e.target.value)} />
      <Input name="password" type="password" placeholder="Holdsport password" value={password} onChange={(e) => setPassword(e.target.value)} />
      <Button onClick={() => getHoldsportTeams()}>Import</Button>
      <ul>
        {data && data.map((team: HoldsportTeam) => (
          <>
            <li key={team.id}>{team.name}</li>
            <Button onClick={() => getTeamActivitiesFromHoldsport({ email, pw: password, teamId: team.id })}>Get activities</Button>
          </>
        ))}
      </ul>
      <ul>
        {activities && activities.map((activity: HoldSportActivity) => {
